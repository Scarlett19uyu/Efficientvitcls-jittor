...
from jittor import transform as transforms  # 使用 Jittor 的 transforms
...
class ColorAug(transforms.ColorJitter, ImageAug):
    ...
    def execute(self, feed_dict: ((dict | np.ndarray) | Image.Image)) -> ((dict | np.ndarray) | Image.Image):
...
class RandAug(ImageAug):
    ...
    def __init__(self, config: dict[(str, Any)], mean: tuple[(float, float, float)], key='data'):
        ...
        aa_params = dict(translate_pct=tpct, img_mean=tuple([min(255, round((255 * x))) for x in mean]), interpolation=Image.BICUBIC)
