...
__all__ = ['apply_drop_func', 'DropPathResidualBlock']
...
def apply_droppath(network: nn.Module, drop_prob: float, linear_decay=True, scheduled=True, skip=0) -> None:
    all_valid_blocks = []
...
class DropPathResidualBlock(ResidualBlock):
    ...
    def execute(self, x: jt.Var) -> jt.Var:
        if not self.is_training() or self.drop_prob == 0 or not isinstance(self.shortcut, IdentityLayer):
        ...
        # Jittor随机数生成
        random_tensor = jt.random(shape, dtype=x.dtype)
