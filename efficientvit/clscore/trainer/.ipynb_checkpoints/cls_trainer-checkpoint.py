...
from efficientvit.models.utils import list_join, list_mean, jt_random_choices
...
class ClsTrainer(Trainer):
    ...
    # mixup
        ...
        # 方案1：使用项目自带的jt_random_choices（推荐）
            if 'jt_random_choices' in globals():
                active_id = jt_random_choices(
                    list(range(len(self.run_config.mixup_config["op"]))),
                    weight_list=mix_weight_list,
            )
        # 方案2：使用numpy的随机选择（兼容性更好）
            else:
                active_id = np.random.choice(
                    len(self.run_config.mixup_config["op"]),
                    p=np.array(mix_weight_list)/sum(mix_weight_list)
                )
                active_id = jt.array(active_id)
            ...
            lam = float(sync_tensor(jt.array(lam), reduce="root"))
    ...
    def run_step(self, feed_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.run_config.mesa is not None and self.run_config.mesa["thresh"] <= self.run_config.progress:
            ...
            ema_output = ema_output.clone()
            ema_output = jt.sigmoid(ema_output).detach()
        else:
            ema_output = None
        ...
        if self.run_config.mixup_config is None:
            top1 = accuracy(output, jt.argmax(labels, dim=1)[0], topk=(1,))[0][0]
        else:
            top1 = None
    ...
    def _train_one_epoch(self, epoch: int) -> dict[str, Any]:
        ...
        with tqdm(
            ...
            for images, labels in self.data_provider.train:
                ...
                # Jittor推荐方式：使用optimizer.step(loss)一步完成
                #output_dict = self.run_step(feed_dict)
                #loss = output_dict["loss"]
                
                # 替换原来的三步操作为一步操作
                #self.optimizer.step(loss)  # 自动执行zero_grad+backward+step
                
                # 或者保持三步操作但修改backward调用方式
                self.optimizer.zero_grad()
                output_dict = self.run_step(feed_dict)
                self.after_step()
    
                # update train metrics
                train_loss.update(output_dict["loss"].item(), images.shape[0])
        ...
        for epoch in range(self.start_epoch, self.run_config.n_epochs + self.run_config.warmup_epochs):
            ...
            if self.auto_restart_thresh is not None:
                if self.best_val - avg_top1 > self.auto_restart_thresh:
                    ...
                    self.load_model(os.path.join(self.checkpoint_path, "model_best.pkl"))
            ...
            if (epoch + 1) % save_freq == 0 or (is_best and self.run_config.progress > 0.8):
                self.save_model(
                    ...
                    epoch=epoch,
                    model_name="model_best.pkl" if is_best else "checkpoint.pkl",
                )